export const BASE_URL : string = 'http://localhost:3002/';
// export const PRODUCT_URL = "http://localhost:3002/products";
// export const CATEGORY_URL = "http://localhost:3002/categories";
// export const SUBGROUP_URL = "http://localhost:3002/subgroups";
// export const SUBCATEGORY_URL = "http://localhost:3002/subcategories";
// export const ORDERS_URL = "http://localhost:3002/orders";
export const LOGIN_URL = "/auth/login";
export const REFRESH_TOKEN_URL = "/auth/refresh-token";